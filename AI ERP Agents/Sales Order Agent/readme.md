...
After importing the sales order agent solution, consider the following to make the agent work for your specific needs and data:
 - **Update the Tools inputs** – open the agent (Sales Order Processor V2), and from the Tools tab open and review all the tools inputs: company code (by default usmf) and mailbox (test email address) must be updated.
 - Open Sales Order Processor V2 Agent - > 
